...
function renderPageNav(numOfStreams){
  let total = 5;
  if(numOfStreams < window.limit){
    total = 1;
  }
  ...
  for(var i=1;i<=total;i++){
    ...
    numDiv.setAttribute('data-num', i - 1);
    numDiv.innerText = i;
    pageContainer.appendChild(numDiv);
  }
...
function renderStreams(streamData){
  ...
  return Promise.all(promises).then(() => {
    return Promise.resolve((streamData.streams || streamData.featured).length);
  });
}
